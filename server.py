...
def det_user_confluence(pair: str, tf: str, highs, lows, opens, closes, times, keys: list) -> Optional[Detected]:
    if not keys:
        return None
    a = atr(highs, lows, closes, 14)
    ...
    if parsed and parsed[-1]:
        today = parsed[-1].date()
        days = sorted({d.date() for d in parsed if d and d.date() < today})
        if days:
            pd = days[-1]
            idxs = [i for i,t in enumerate(parsed) if t and t.date()==pd]
            pd_high = max(highs[min(idxs):max(idxs)+1]); pd_low = min(lows[min(idxs):max(idxs)+1]); pd_close = closes[max(idxs)]
            P = (pd_high + pd_low + pd_close)/3.0; R1 = 2*P - pd_low; S1 = 2*P - pd_high
            pd_levels = (pd_high, pd_low, P, R1, S1)
    K, Dv, RS = _stoch_rsi_kd(closes, 14, 14, 3, 3)
    ...
    checks = {
        'trend_structure_any': slope50 != 0,
        'd1_h4_bias_any': (e50[-1] != e200[-1]),
        'ema2050_any': ( (e20[-1] > e50[-1] and slope50 > 0) or (e20[-1] < e50[-1] and slope50 < 0) ),
        'location_any': _fib_overlap(closes[-1], highs, lows, 50, 0.001),
        'htf_sr_any': (at_sr_long or at_sr_short),
        'sd_any': (at_sr_long or at_sr_short),
        'fib_overlap': _fib_overlap(closes[-1], highs, lows, 50, 0.001),
        'round_level': _round_level_proximity(closes[-1], pair),
        'pdh_or_pdl': False,
        'classic_pivots': False,
        'liq_imbalance': False,
        'liq_sweep_any': (sweep in ('long','short')),
        'fvg_any': bool(has_fvg),
        'momentum_any': (mom_long or mom_short),
        'pinbar_any': bool(d_pin),
        'engulf_any': bool(d_eng),
        'hammer_or_shooting': False,
        'morning_or_evening': (ms or es),
        'hanging_man': False,
        'pinbar_long': bool(d_pin and d_pin.direction=='long'),
        'pinbar_short': bool(d_pin and d_pin.direction=='short'),
        'engulf_long': bool(d_eng and d_eng.direction=='long'),
        'engulf_short': bool(d_eng and d_eng.direction=='short'),
        'momentum_long': mom_long,
        'momentum_short': mom_short,
    }
    checks['hammer_or_shooting'] = checks['pinbar_long'] or checks['pinbar_short']
    checks['hanging_man'] = checks['pinbar_short'] and slope50 > 0

    if pd_levels:
        pdh, pdl, P, R1, S1 = pd_levels
    ...
    checks['liq_imbalance'] = eq_highs or eq_lows or checks['fvg_any']

    non_candle_map = {
        "Trend and structure":"trend_structure_any",
        "D1/H4 bias":"d1_h4_bias_any",
        "20/50 EMA slope/stack":"ema2050_any",
        "Location":"location_any",
        "HTF S/R":"htf_sr_any",
        "Supply - Demand zones":"sd_any",
        ...
        "Liquidity sweep":"liq_sweep_any",
        "Fair Value Gap":"fvg_any",
        "Momentum Confirmation":"momentum_any",
    }
    candle_map = {
        "Pinbar": "pinbar_any",
        "Engulfing": "engulf_any",
        "Hammer / Shooting star": "hammer_or_shooting",
        "Morning star / Evening star": "morning_or_evening",
        "Hanging man": "hanging_man",
    }

    selected = list(keys)
    need_non = [non_candle_map[k] for k in selected if k in non_candle_map]
    need_cand = [candle_map[k] for k in selected if k in candle_map]

    ok_non = all(checks.get(k, False) for k in need_non) if need_non else True
    ok_cand = any(checks.get(k, False) for k in need_cand) if need_cand else True

    if not (ok_non and ok_cand):
        return None

    if checks['pinbar_long'] or checks['engulf_long'] or checks['momentum_long'] or (sweep=='long'):
        direction = 'long'
    elif checks['pinbar_short'] or checks['engulf_short'] or checks['momentum_short'] or (sweep=='short'):
        direction = 'short'
    else:
        direction = 'long' if slope50 >= 0 else 'short'

    entry = closes[-1]
    stop  = lows[-1] if direction=='long' else highs[-1]
    pts = 0
    pts += sum(1 for k in need_non if checks.get(k, False))
    if need_cand and ok_cand: pts += 1
