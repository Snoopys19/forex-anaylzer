...
# === AstraFX Strategy Hotfix (auto-discovery + debug) =========================
from fastapi import Body
...
def _list_strategy_files():
    try:
        return [fn for fn in os.listdir(STRATEGY_DIR_PATH)]
    except Exception as e:
        return ["<error listing strategies: %s>" % e]

def _load_strategy_meta():
    metas = []
    for fn in _list_strategy_files():
        if fn.endswith(".json"):
            fp = os.path.join(STRATEGY_DIR_PATH, fn)
            try:
                with open(fp, "r") as f:
                    meta = json.load(f)
                    metas.append(meta)
            except Exception as e:
                print("Strategy JSON load failed:", fp, e)
    return metas
...
def api_strategies():
    ...
    print("[AstraFX] strategies loaded:", [m.get("id") for m in metas])
    return metas

@app.get("/api/strategies/debug")
def api_strategies_debug():
    return {"cwd": os.getcwd(), "strategy_dir": STRATEGY_DIR_PATH, "files": _list_strategy_files()}
...
def api_tester_json(payload: dict = Body(...)):
    ...
    if sid not in metas:
        return {"error": f"strategy_id not found: {sid}", "known": list(metas.keys())}
    meta = metas[sid]
    ...
    if not bars:
        return {"error": "No OHLC returned", "pair": pair, "timeframe": tf}
...
try:
    @app.post("/api/tester")
    def api_tester_compat(payload: dict = Body(...)):
        if "strategy_id" in payload:
            return api_tester_json(payload)
        return {"info": "strategy_id not provided; use /api/tester_json or include strategy_id"}
except Exception as _e:
    print("Compat /api/tester hook skipped:", _e)
