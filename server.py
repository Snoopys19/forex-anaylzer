...
def api_backtest(payload: Dict[str, Any] = Body(...)):
    """
    30-day backtest (closed bars only). Per-signal filters for strategy/pattern/proprietary/udc.
    """
    try:
        ...
        def first_touch_outcome(dir_: str, entry: float, stop: float, tps: list, fwd: list, rule: str):
            risk = abs(entry - (stop if stop is not None else entry))
            ru = str(rule).upper()
            tp = None
            if ru.startswith("TP"):
                try:
                    idx = max(0, min(2, int(ru[2:]) - 1))
                except Exception:
                    idx = 0
                if tps and len(tps)>idx: tp = tps[idx]
            elif ru.startswith("FIXEDRR:"):
                try:
                    rr = float(ru.split(":",1)[1])
            ...
            for j, bar in enumerate(fwd, start=1):
                ...
                if stop_hit and tp_hit and risk>0:
                    if abs(entry-(stop if stop is not None else entry)) <= abs((tp if tp is not None else entry)-entry):
                        return ("loss", j, -1.0)
                    else:
        ...
        rr_acc = 0.0
        def allow(sel, name): return True if not sel else (name or "").lower() in sel
            ...
            for tf in tfs:
                ...
                for i in range(warmup, len(series)-1):
                    ...
                    for d in (cands.get("strategy") or []):
                        if allow(f_strategy, d.name) and d.score >= min_score:
                            sig = d.to_signal(pair, tf)
                            o,b,rr = first_touch_outcome(sig["dir"], sig["entry"], sig["stop"], sig["tps"], series[i+1:i+1+outcome_bars], exit_rule)
                            ...
                            if o=='win': rec["wins"]+=1; summaries["wins"]+=1; p_sum["wins"]+=1; rr_acc+=rr
                            elif o=='loss': rec["losses"]+=1; summaries["losses"]+=1; p_sum["losses"]+=1; rr_acc+=rr
                            else: rec["timeouts"]+=1; summaries["timeouts"]+=1; p_sum["timeouts"]+=1

                    for d in (cands.get("pattern") or []):
                        if allow(f_pattern, d.name) and d.score >= min_score:
                            sig = d.to_signal(pair, tf)
                            o,b,rr = first_touch_outcome(sig["dir"], sig["entry"], sig["stop"], sig["tps"], series[i+1:i+1+outcome_bars], exit_rule)
        ...
        if summaries["total"]>0: summaries["avg_rr"]=rr_acc/summaries["total"]
        return summaries
    except HTTPException:
        raise
    except Exception as e:
