# AstraFX Backend v2.3 — Gating-only edition
# Adds a light gate to /app using a signed HttpOnly cookie set by /api/signup or /api/login.
#
# ENV (required/optional):
#   OANDA_TOKEN (required), OANDA_ENV=practice|live
#   SESSION_SECRET (required in prod), COOKIE_NAME, COOKIE_SECURE
#   [optional commit signups to repo] GITHUB_TOKEN, GITHUB_REPO, GITHUB_BRANCH
import os, base64, hmac, time, datetime
...
class Detected:
    def __init__(self, name: str, direction: str, entry: float, stop: float, score: float, tags: List[str]):
        ...
        self.entry = float(entry); self.stop = float(stop); self.score = float(score); self.tags = tags
    def to_signal(self, pair: str, tf: str) -> Dict[str, Any]:
        ...
        tv_sym = f"OANDA:{pair}"
        return {"pair": pair, "tf": tf, "score": int(round(self.score)), "setup": self.name, "dir": self.direction,
                "entry": self.entry, "stop": self.stop, "tps": [float(x) for x in tps], "tags": self.tags[:],
...
def detect_pinbar(highs, lows, opens, closes) -> Optional[Detected]:
    i = -1; o, h, l, c = opens[i], highs[i], lows[i], closes[i]
    body = abs(c - o); 
    if body == 0: return None
    rng = h - l; up = c > o
    upper = h - max(c, o); lower = min(c, o) - l
    tail = lower if up else upper; tail_ratio = tail / (body + 1e-9)
    if tail_ratio < 2.0 or body / (rng + 1e-9) > 0.4: return None
    direction = "long" if up and lower > upper else "short" if (not up) and upper > lower else None
    if not direction: return None
    entry = c; stop = l if direction == "long" else h; score = min(100, 60 + tail_ratio * 8)
...
def detect_engulfing(highs, lows, opens, closes) -> Optional[Detected]:
    ...
    o1, c1 = opens[-2], closes[-2]; o2, c2 = opens[-1], closes[-1]
    body1 = abs(c1 - o1); body2 = abs(c2 - o2)
    min1, max1 = min(o1, c1), max(o1, c1); min2, max2 = min(o2, c2), max(o2, c2)
    if not (min2 <= min1 and max2 >= max1 and body2 > body1 * 0.8): return None
    direction = "long" if c2 > o2 and c1 < o1 else "short" if c2 < o2 and c1 > o1 else None
    if not direction: return None
    entry = c2; stop = lows[-2] if direction == "long" else highs[-2]
...
def detect_insidebar(highs, lows, opens, closes) -> Optional[Detected]:
    ...
    if highs[-1] <= highs[-2] and lows[-1] >= lows[-2]:
        ...
        entry = closes[-1]; stop = lows[-2] if direction == "long" else highs[-2]
...
def detect_nr7(highs, lows, opens, closes) -> Optional[Detected]:
    if len(highs) < 7: return None
    rng = [h - l for h, l in zip(highs, lows)]
    if rng[-1] > min(rng[-7:]): return None
    direction = "long" if closes[-1] >= opens[-1] else "short"
    entry = closes[-1]; stop = lows[-1] if direction == "long" else highs[-1]
...
def detect_donchian20(highs, lows, closes) -> Optional[Detected]:
    ...
    hi20 = highest(highs, 20); lo20 = lowest(lows, 20); c = closes[-1]
    if c > hi20:
        ...
        return None
    ema50 = ema(closes, 50); slope = ema50[-1] - ema50[-5] if len(ema50) > 5 else 0.0
    atr14 = atr(highs, lows, closes, 14)
    dist = abs(c - (hi20 if direction == "long" else lo20)); vol = atr14[-1] if atr14[-1] else 1e-9
...
def detect_rsi_pullback(highs, lows, closes) -> Optional[Detected]:
    ...
    ema50 = ema(closes, 50); slope = ema50[-1] - ema50[-5] if len(ema50) > 5 else 0.0
    rs = rsi(closes, 14); c = closes[-1]; recent = rs[-10:]
    if slope > 0 and min(recent) < 45 and rs[-1] > 50:
        direction = "long"; entry = c; stop = lowest(lows, 10)
        conf = (rs[-1] - 50) + (slope / (abs(c) + 1e-9)) * 1000; score = min(100, 68 + conf * 0.2)
        return Detected("RSI Pullback (Uptrend)", direction, entry, stop, score, tags=["rsi"])
    if slope < 0 and max(recent) > 55 and rs[-1] < 50:
        direction = "short"; entry = c; stop = highest(highs, 10)
        conf = (50 - rs[-1]) + (-slope / (abs(c) + 1e-9)) * 1000; score = min(100, 68 + conf * 0.2)
...
def det_bbsqueeze_strategy(highs, lows, closes) -> Optional[Detected]:
    ...
    upper = [m + 2*s for m, s in zip(mid, sd)]; lower = [m - 2*s for m, s in zip(mid, sd)]
    bandwidth = [(u - l) / (m if m != 0 else 1e-9) for u, l, m in zip(upper, lower, mid)]
    if bandwidth[-1] > percentile(bandwidth[-40:], 20): return None
    c = closes[-1]
    if c > upper[-1]: direction, entry, stop = "long", c, mid[-1]
    elif c < lower[-1]: direction, entry, stop = "short", c, mid[-1]
    else: return None
    atr14 = atr(highs, lows, closes, 14); dist = abs(c - (upper[-1] if direction == "long" else lower[-1])); vol = atr14[-1] or 1e-9
    score = min(100, 68 + (dist / vol) * 12); return Detected(strategy_label("bbsqueeze"), direction, entry, stop, score, tags=["bbsqueeze"])

def det_donchian_strategy(highs, lows, closes) -> Optional[Detected]:
    d = detect_donchian20(highs, lows, closes); 
    if not d: return None
    d.name = strategy_label("donchian"); return d
...
def det_ema_pullback_strategy(highs, lows, closes) -> Optional[Detected]:
    ...
    def crossed_up(ea, eb, look=10):
        for i in range(-look, 0):
            if ea[i-1] <= eb[i-1] and ea[i] > eb[i]: return True
        return False
    def crossed_down(ea, eb, look=10):
        ...
        return False
    c = closes[-1]
    if e9[-1] > e21[-1] and (crossed_up(e9, e21) or c > e9[-1]): direction, entry, stop = "long", c, lowest(lows, 10)
    elif e9[-1] < e21[-1] and (crossed_down(e9, e21) or c < e9[-1]): direction, entry, stop = "short", c, highest(highs, 10)
    else: return None
...
def det_ichimoku_strategy(highs, lows, closes) -> Optional[Detected]:
    ...
    c = closes[-1]
    if c > cloud_top[-1] and tenkan[-1] > kijun[-1]: direction, entry, stop = "long", c, min(kijun[-1], cloud_bot[-1])
    elif c < cloud_bot[-1] and tenkan[-1] < kijun[-1]: direction, entry, stop = "short", c, max(kijun[-1], cloud_top[-1])
    else: return None
    score = 70; return Detected(strategy_label("ichimoku"), direction, entry, stop, score, tags=["ichimoku"])

def det_orb_london_strategy(times: List[str], highs, lows, closes, tf: str) -> Optional[Detected]:
    if tf not in {"M5","M15","M30"}: return None
    import datetime as dt
    try: parsed = [dt.datetime.fromisoformat(t.replace("Z","+00:00")) for t in times]
    except Exception: return None
    today = parsed[-1].date(); idx = [i for i,x in enumerate(parsed) if x.date() == today]
    if not idx: return None
    first = idx[0]; window_bars = {"M5":6, "M15":2, "M30":1}[tf]
    end = min(len(highs), first + window_bars); or_high = max(highs[first:end]); or_low = min(lows[first:end]); c = closes[-1]
    if c > or_high: direction, entry, stop = "long", c, or_low
    elif c < or_low: direction, entry, stop = "short", c, or_high
    else: return None
    score = 65; return Detected(strategy_label("orb_london"), direction, entry, stop, score, tags=["orb_london"])
...
def det_rsi2_strategy(highs, lows, closes) -> Optional[Detected]:
    ...
    if c > s200[-1] and r2[-1] < 10: direction, entry, stop = "long", c, lowest(lows, 10)
    elif c < s200[-1] and r2[-1] > 90: direction, entry, stop = "short", c, highest(highs, 10)
    else: return None
...
def gather_candidates(pair: str, tf: str, series: List[Dict[str, float]], which_pattern: str, which_strategy: str) -> Dict[str, List[Detected]]:
    opens  = [x["open"] for x in series]; highs  = [x["high"] for x in series]
    lows   = [x["low"] for x in series]; closes = [x["close"] for x in series]
    times  = [x.get("time","") for x in series]
    ws = (which_strategy or "all").lower(); wp = (which_pattern or "all").lower()
    def tf_allowed(name_key: str) -> bool: return tf in STRATEGY_TF_MAP.get(name_key, set())
    strat: List[Detected] = []
    ...
    pat: List[Detected] = []
    (lambda d=detect_pinbar(highs, lows, opens, closes): pat.append(d) if d else None)() if wp in ("all","pinbar") else None
    (lambda d=detect_engulfing(highs, lows, opens, closes): pat.append(d) if d else None)() if wp in ("all","engulfing") else None
    (lambda d=detect_insidebar(highs, lows, opens, closes): pat.append(d) if d else None)() if wp in ("all","insidebar") else None
    (lambda d=detect_nr7(highs, lows, opens, closes): pat.append(d) if d else None)() if wp in ("all","nr7","NR7") else None
    (lambda d=detect_donchian20(highs, lows, closes): pat.append(d) if d else None)() if wp in ("all","donchian20") else None
    (lambda d=detect_rsi_pullback(highs, lows, closes): pat.append(d) if d else None)() if wp in ("all","rsi") else None
...
def best_or_union(pair: str, tf: str, series: List[Dict[str, float]], which_pattern: str, which_strategy: str, min_score: int) -> List[Dict[str, Any]]:
    ...
    if ws != "all" and wp != "all":
        out: List[Detected] = []
        if c["strategy"]: out.append(max(c["strategy"], key=lambda d: d.score))
        if c["pattern"]:  out.append(max(c["pattern"],  key=lambda d: d.score))
        uniq = {}; 
        for d in out: uniq[(d.name, d.direction, round(d.entry,6), round(d.stop,6))] = d
...
COOKIE_SECURE  = os.environ.get("COOKIE_SECURE", "1") != "0"  # set "0" for local HTTP

def _is_email_registered(email: str) -> bool:
    try:
        with open(SIGNUP_LOCAL_PATH, "r") as f:
            for line in f:
                if "," in line and line.strip().split(",")[-1].strip().lower() == email.lower():
                    return True
...
def _require_session(request: Request) -> str:
    ...
    if not email or not _is_email_registered(email): return ""
    return email

@app.post("/api/signup")
def api_signup(payload: Dict[str, Any] = Body(...), response: Response = None):
    ...
    try:
        with open(SIGNUP_LOCAL_PATH, "rb") as f: data_bytes = f.read()
...
def api_login(payload: Dict[str, Any] = Body(...), response: Response = None):
    email = (payload.get("email") or "").strip()
    if not email or "@" not in email: raise HTTPException(status_code=400, detail="Valid email required")
    if not _is_email_registered(email): raise HTTPException(status_code=403, detail="Email not found. Please sign up first.")
    if response is None:
        from fastapi import Response as _Resp
        response = _Resp()
    _set_session_cookie(response, email)
