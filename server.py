...
# === AI Trading Ideas Support ===
# Dataclass for a discretionary trading idea. This container stores all the
# information needed to construct a user‑friendly plan card on the frontend.
@dataclass
class AIIdea:
    pair: str
    tf: str
...
# Load high‑impact news events from a local JSON file. The path can be
# overridden to point to any file under `public/data`. Events should be
# dictionaries with keys: time (ISO string with Z), currency, impact, title.
def load_local_news_events(path: str = "public/data/news.json") -> List[Dict[str, Any]]:
...
# Parse an ISO timestamp (Z suffix) to a naive UTC datetime. Falls back to
# current time on failure.
def _to_dt(ts: str) -> dt.datetime:
    try:
        return dt.datetime.fromisoformat(ts.replace("Z", "+00:00"))
    except Exception:
        return dt.datetime.utcnow()

def det_ai_trading_idea(
    pair: str, tf: str,
    highs: List[float], lows: List[float], opens: List[float], closes: List[float], times: List[str],
    news_events: List[Dict[str, Any]]
) -> Optional[AIIdea]:
    """Generate an AI trading idea from technical context on the last closed bar.

    This method synthesizes indicators such as moving averages, ATR, swing
    levels, FVGs, liquidity sweeps, candlestick patterns and momentum resets
    into a directional bias and trade plan. If the setup lacks sufficient
    confluence or clarity, returns None.
    """
    if len(closes) < 60:
        return None
    # Indicators
    ...
    # Swing levels for recent 50 bars
    ...
    fvg_present, fvg_dir = _fvg_context(highs, lows)
    liquidity = _liquidity_sweep(highs, lows, closes, 10)
    pin = detect_pinbar(highs, lows, opens, closes)
    eng = detect_engulfing(highs, lows, opens, closes)
    # Momentum via Stoch RSI
    ...
    if down_trend:
        bias = "short"
    # Neutralize if near opposite SR
    ...
    long_trig = False
    short_trig = False
    if pin and getattr(pin, "direction", "") == "long":
        long_trig = True
    if pin and getattr(pin, "direction", "") == "short":
        short_trig = True
    if eng and getattr(eng, "direction", "") == "long":
        long_trig = True
    if eng and getattr(eng, "direction", "") == "short":
        short_trig = True
    if mom_long:
        long_trig = True
    if mom_short:
    ...
    if direction == "neutral":
        return None
    # Construct entry, stop and targets
    close = closes[-1]
    if direction == "long":
        stop = min(lows[-1], lows[-2] if len(lows) > 1 else lows[-1])
        stop = min(stop, close - a14[-1] * 0.8)
        tp1 = close + (close - stop) * 1.5
        tp2 = close + (close - stop) * 2.0
    else:
        stop = max(highs[-1], highs[-2] if len(highs) > 1 else highs[-1])
    ...
    if liquidity:
        conf.append(f"Liquidity sweep ({liquidity})")
    if fvg_present:
    ...
    # Score calculation: base 60 + 5 per unique confluence up to 95
    score = 60
    ...
    # Session note (placeholder, could be refined)
    session_note = "Session OK"
    # News warning
    news_warning = None
    if news_events:
        try:
        ...
        for ev in news_events:
            if ev["impact"] in ("high", "red") and ev["ccy"] in (base_ccy, quote_ccy):
                diff = (ev["time"] - now).total_seconds()
                if abs(diff) <= window.total_seconds():
                    mins = int(diff / 60)
                    news_warning = f"{ev['ccy']} {ev['title']} in {mins}m" if mins >= 0 else f"{ev['ccy']} {ev['title']} {abs(mins)}m ago"
                    break
    # Rationale string summarizing confluences
    rationale_bits: List[str] = []
    if direction == "long":
        if up_trend:
            rationale_bits.append("Uptrend (EMA20>EMA50, rising 50)")
        if near_sr_long:
            rationale_bits.append("Rejected support")
        if liquidity == "long":
            rationale_bits.append("Liquidity sweep of lows and close back in")
        if fvg_present:
            rationale_bits.append("FVG fill into level")
        if pin and getattr(pin, "direction", "") == "long":
            rationale_bits.append("Bullish pin bar trigger")
        if eng and getattr(eng, "direction", "") == "long":
            rationale_bits.append("Bullish engulfing trigger")
        if mom_long:
            rationale_bits.append("Momentum reset from oversold (Stoch RSI)")
    else:
        if down_trend:
            rationale_bits.append("Downtrend (EMA20<EMA50, falling 50)")
        if near_sr_short:
            rationale_bits.append("Rejected resistance")
        if liquidity == "short":
            rationale_bits.append("Liquidity sweep of highs and close back in")
        if fvg_present:
            rationale_bits.append("FVG fill into level")
        if pin and getattr(pin, "direction", "") == "short":
            rationale_bits.append("Bearish pin bar trigger")
        if eng and getattr(eng, "direction", "") == "short":
            rationale_bits.append("Bearish engulfing trigger")
        if mom_short:
            rationale_bits.append("Momentum reset from overbought (Stoch RSI)")
    rationale = "; ".join(rationale_bits) if rationale_bits else "Multiple technical confluences."
    return AIIdea(
        pair=pair,
        tf=tf,
        direction=direction,
...
def api_scan(
    ...
    udc_items: List[str] = []
    if udc:
        # Allow comma‑separated list of items; ignore blanks
        udc_items = [x.strip() for x in udc.split(",") if x.strip()]
    # Determine whether to generate AI ideas. Accept '1', 'true', or 'on'.
    ai_flag = False
    ai_param = (ai or "0").lower()
    if ai_param in ("1", "true", "on"): ai_flag = True
    # Preload news events once if AI mode is on
    news_events: List[Dict[str, Any]] = []
    if ai_flag:
        news_events = load_local_news_events()
    for p in [x.strip() for x in pairs.split(",") if x.strip()]:
        try:
            ...
            # User‑defined confluence detection (append separately)
            if udc_items:
                # Extract OHLC arrays if not already prepared
                opens  = [x["open"] for x in series]
                highs  = [x["high"] for x in series]
                lows   = [x["low"] for x in series]
                closes = [x["close"] for x in series]
            ...
            # AI trading idea detection (append separately)
            if ai_flag:
                # Extract arrays if not yet defined
                opens_arr = [x["open"] for x in series]
                highs_arr = [x["high"] for x in series]
                lows_arr  = [x["low"] for x in series]
                closes_arr= [x["close"] for x in series]
                times_arr = [x.get("time", "") for x in series]
                idea = det_ai_trading_idea(p, tf, highs_arr, lows_arr, opens_arr, closes_arr, times_arr, news_events)
                if idea and idea.score >= min_score:
                    # Convert idea to response dict for frontend
                    results.append({
                        "type": "ai_idea",
                        "pair": idea.pair,
                        "tf": idea.tf,
                        ...
                        "targets": [float(x) for x in idea.targets],
                        "rr": idea.rr,
                        "confluences": idea.confluences,
                        "rationale": idea.rationale,
                        "session_note": idea.session_note,
                        "news_warning": idea.news_warning,
                        "score": idea.score
                    })
        except Exception as e:
