...
def det_astra1_strategy(pair: str, tf: str, highs, lows, opens, closes, times) -> Optional[Detected]:
    ...
    return Detected("AstraFX Strategy #1", direction, entry, stop, score, tags=["astra1", "confluence"])
...
def det_user_confluence(pair: str, tf: str, highs, lows, opens, closes, times, keys: List[str]) -> Optional[Detected]:
    ...
    mid_ok = _fib_overlap(closes[-1], highs, lows, 50, 0.001)
    checks['location_any'] = mid_ok
    ...
    ok = all(key_to_check(k) for k in keys)
    if not ok:
        return None
