...
def det_ai_trading_idea(pair: str, tf: str,
    ...
    # Narrative sections
    if direction == "long":
        bias_text = "Long — Uptrend (EMA20>EMA50, rising 50) with valid location."
        notes_text = "Move to BE at TP1 or after HH; skip if price closes below invalidation."
    else:
        bias_text = "Short — Downtrend (EMA20<EMA50, falling 50) with valid location."
        notes_text = "Move to BE at TP1 or after LL; skip if price closes above invalidation."
    plan_text = f"Entry {close:.5f}; Stop {stop:.5f}; TP1 {tp1:.5f}; TP2 {tp2:.5f}; R:R ≈ {rr}"
