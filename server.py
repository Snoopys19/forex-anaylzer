...
def api_backtest(payload: Dict[str, Any] = Body(...)):
    """
    30-day backtest on closed bars using existing detectors.
    Filters allow per-signal selection (strategy/pattern/proprietary/udc).
    """
    try:
        import re as _re
        pairs = payload.get("pairs") or []
        tfs = payload.get("timeframes") or []
        lookback = int(payload.get("lookbackDays") or 30)
        lookback = max(1, min(lookback, 30))
        min_score = int(payload.get("min_score") or 70)
        filters = payload.get("filters") or {}
        f_strategy = [x.lower() for x in filters.get("strategy", []) if x]
        f_pattern  = [x.lower() for x in filters.get("pattern", []) if x]
        f_prop     = [x.lower() for x in filters.get("proprietary", []) if x]
        f_udc      = [x.lower() for x in filters.get("udc", []) if x]
        outcome_bars = int(payload.get("outcomeBars") or 60)
        exit_rule = str(payload.get("exitRule") or "TP1").upper()

        def bars_for_tf(tf: str, days: int) -> int:
            tf = tf.upper()
            if tf == "M5":  n = days*24*12
            elif tf == "M15": n = days*24*4
            elif tf == "M30": n = days*24*2
            elif tf == "H1": n = days*24
            elif tf == "H4": n = days*6
            elif tf == "D1": n = days
            else: n = days*24
            return int(min(max(n+100, 200), 4000))

        def first_touch_outcome(dir_: str, entry: float, stop: float, tps: list, fwd: list, rule: str):
            risk = abs(entry - (stop or entry))
            # choose TP based on rule
            if str(rule).upper().startswith("TP"):
                try:
                    idx = max(0, min(2, int(str(rule)[2:]) - 1))
                except Exception:
                    idx = 0
                tp = tps[idx] if (tps and len(tps)>idx) else None
            elif str(rule).upper().startswith("FIXEDRR:"):
                try:
                    rr = float(str(rule).split(":",1)[1])
                    tp = entry + (risk*rr if dir_ == "long" else -risk*rr)
                except Exception:
                    tp = tps[0] if tps else None
            else:
                tp = tps[0] if tps else None

            for j, bar in enumerate(fwd, start=1):
                ...
                if stop_hit and tp_hit and risk>0:
                    # tie-breaker: whichever is closer to entry wins
                    if abs(entry-(stop or entry)) <= abs((tp or entry)-entry):
                        return ("loss", j, -1.0)
                    else:
                        return ("win", j, abs((tp-entry)/risk) if risk>0 else 0.0)
                if stop_hit: return ("loss", j, -1.0)
                if tp_hit:   return ("win", j, abs((tp-entry)/risk) if risk>0 else 0.0)
                if j >= outcome_bars: break
            return ("timeout", outcome_bars, 0.0)

        summaries = {"total":0,"wins":0,"losses":0,"timeouts":0,"avg_rr":0.0,"per_signal":{},"per_pair":{}}
        rr_acc = 0.0

        def allow_name(sel: list, name: str) -> bool:
            if not sel: return True  # empty means 'All'
            key = (name or "").lower()
            return (key in sel)

        for pair in pairs:
            p_sum = summaries["per_pair"].setdefault(pair, {"total":0,"wins":0,"losses":0,"timeouts":0})
            for tf in tfs:
                limit = bars_for_tf(tf, lookback)
                series = fetch_ohlc_router(pair, tf, limit)
                opens  = [x["open"] for x in series]
                highs  = [x["high"] for x in series]
                lows   = [x["low"] for x in series]
                closes = [x["close"] for x in series]
                ...
                for i in range(warmup, len(series)-1):
                    ...
                    # built-in strategies/patterns
                    cands = gather_candidates(pair, tf, window, which_pattern="all", which_strategy="all")

                    # Strategies
                    for d in cands.get("strategy") or []:
                        if allow_name(f_strategy, d.name) and d.score >= min_score:
                            sig = d.to_signal(pair, tf)
                            outcome, bars_to, rr = first_touch_outcome(sig["dir"], sig["entry"], sig["stop"], sig["tps"], series[i+1:i+1+outcome_bars], exit_rule)
                            key = f"strategy:{(d.name or '').lower()}"
                            rec = summaries["per_signal"].setdefault(key, {"name": d.name, "group":"strategy","total":0,"wins":0,"losses":0,"timeouts":0})
                            rec["total"]+=1; summaries["total"]+=1; p_sum["total"]+=1
                            if outcome=="win": rec["wins"]+=1; summaries["wins"]+=1; p_sum["wins"]+=1; rr_acc+=rr
                            elif outcome=="loss": rec["losses"]+=1; summaries["losses"]+=1; p_sum["losses"]+=1; rr_acc+=rr
                            else: rec["timeouts"]+=1; summaries["timeouts"]+=1; p_sum["timeouts"]+=1

                    # Patterns
                    for d in cands.get("pattern") or []:
                        if allow_name(f_pattern, d.name) and d.score >= min_score:
                            sig = d.to_signal(pair, tf)
                            outcome, bars_to, rr = first_touch_outcome(sig["dir"], sig["entry"], sig["stop"], sig["tps"], series[i+1:i+1+outcome_bars], exit_rule)
                            key = f"pattern:{(d.name or '').lower()}"
                            rec = summaries["per_signal"].setdefault(key, {"name": d.name, "group":"pattern","total":0,"wins":0,"losses":0,"timeouts":0})
                            rec["total"]+=1; summaries["total"]+=1; p_sum["total"]+=1
                            if outcome=="win": rec["wins"]+=1; summaries["wins"]+=1; p_sum["wins"]+=1; rr_acc+=rr
                            elif outcome=="loss": rec["losses"]+=1; summaries["losses"]+=1; p_sum["losses"]+=1; rr_acc+=rr
                            else: rec["timeouts"]+=1; summaries["timeouts"]+=1; p_sum["timeouts"]+=1

                    # Proprietary (expandable later; for now handle astra1 if selected or if none selected = all)
                    want_astra1 = (not f_prop) or ("astra1" in f_prop)
                    if want_astra1:
                        dprop = det_astra1_strategy(pair, tf, highs[:i+1], lows[:i+1], opens[:i+1], closes[:i+1], times[:i+1])
                        if dprop and dprop.score >= min_score:
                            sig = dprop.to_signal(pair, tf)
                            outcome, bars_to, rr = first_touch_outcome(sig["dir"], sig["entry"], sig["stop"], sig["tps"], series[i+1:i+1+outcome_bars], exit_rule)
                            key = "proprietary:astra1"
                            rec = summaries["per_signal"].setdefault(key, {"name": dprop.name, "group":"proprietary","total":0,"wins":0,"losses":0,"timeouts":0})
                            rec["total"]+=1; summaries["total"]+=1; p_sum["total"]+=1
                            if outcome=="win": rec["wins"]+=1; summaries["wins"]+=1; p_sum["wins"]+=1; rr_acc+=rr
                            elif outcome=="loss": rec["losses"]+=1; summaries["losses"]+=1; p_sum["losses"]+=1; rr_acc+=rr
                            else: rec["timeouts"]+=1; summaries["timeouts"]+=1; p_sum["timeouts"]+=1

                    # UDC — only if user selected at least one UDC item
                    if f_udc:
                        dudc = det_user_confluence(pair, tf, highs[:i+1], lows[:i+1], opens[:i+1], closes[:i+1], times[:i+1], f_udc)
                        if dudc and dudc.score >= min_score:
                            sig = dudc.to_signal(pair, tf)
                            outcome, bars_to, rr = first_touch_outcome(sig["dir"], sig["entry"], sig["stop"], sig["tps"], series[i+1:i+1+outcome_bars], exit_rule)
                            key = "udc:combo"
                            rec = summaries["per_signal"].setdefault(key, {"name": dudc.name, "group":"udc","total":0,"wins":0,"losses":0,"timeouts":0})
                            rec["total"]+=1; summaries["total"]+=1; p_sum["total"]+=1
                            if outcome=="win": rec["wins"]+=1; summaries["wins"]+=1; p_sum["wins"]+=1; rr_acc+=rr
                            elif outcome=="loss": rec["losses"]+=1; summaries["losses"]+=1; p_sum["losses"]+=1; rr_acc+=rr
                            else: rec["timeouts"]+=1; summaries["timeouts"]+=1; p_sum["timeouts"]+=1
        ...
        if summaries["total"]>0:
            summaries["avg_rr"] = rr_acc / summaries["total"]
        return summaries
