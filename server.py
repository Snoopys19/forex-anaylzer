...
# --- AI Trading Ideas imports ---
from dataclasses import dataclass
from typing import List, Optional  # re-import List and Optional for AI dataclass
import datetime as dt
import json
import random

# === AI Trading Ideas model and helpers ===
@dataclass
class AIIdea:
    pair: str
    tf: str
...
def load_local_news_events(path: str = "public/data/news.json"):
    """
    Load a simple list of upcoming news events from a JSON file.
    Each entry should contain an ISO timestamp (`time`), `currency`, `impact` (e.g., 'high'), and a short `title`.
    The timestamps must be in UTC (e.g. "2025-09-10T12:30:00Z").
    Returns a list of dicts with parsed datetime objects.
    """
    try:
        ...
        for e in raw:
            try:
                ...
                out.append({"time": when, "ccy": e.get("currency", ""), "impact": e.get("impact", ""), "title": e.get("title", "")})
            except Exception:
                continue
        return out
    except Exception:
...
def det_ai_trading_idea(pair: str, tf: str,
                        highs: List[float], lows: List[float], opens: List[float], closes: List[float], times: List[str], news_events: List[Dict[str, Any]]) -> Optional[AIIdea]:
    """
    Generate a discretionary AI trading idea based on the most recent closed bar and context.
    It evaluates trend, location, liquidity, momentum and candlestick triggers, then constructs
    a narrative plan with entry, stop and targets. Returns None if no reasonable idea forms.
    """
    if len(closes) < 60:
        return None
    ...
    up_trend = (e20[-1] > e50[-1] and slope50 > 0)
    down_trend = (e20[-1] < e50[-1] and slope50 < 0)

    # Location & context using existing helpers
    hh, ll = _swing_levels(highs, lows, 50)
    tol = max(a14[-1] * 0.25, closes[-1] * 0.0005)
    near_sr_long = _near(closes[-1], ll, tol)
    near_sr_short = _near(closes[-1], hh, tol)
    fvg, fvg_dir = _fvg_context(highs, lows)
    sweep = _liquidity_sweep(highs, lows, closes, 10)  # 'long'|'short'|''

    # Candlestick triggers
    pin = detect_pinbar(highs, lows, opens, closes)
    eng = detect_engulfing(highs, lows, opens, closes)

    # Stoch RSI (close-based) for momentum reset
    K, Dv, _ = _stoch_rsi_kd(closes, 14, 14, 3, 3)
    k_prev, d_prev = (K[-2] if len(K) > 1 else 50.0), (Dv[-2] if len(Dv) > 1 else 50.0)
    k_last, d_last = K[-1], Dv[-1]
    mom_long = k_prev < 20 and k_last > 20 and k_last > d_last and k_prev <= d_prev
    mom_short = k_prev > 80 and k_last < 80 and k_last < d_last and k_prev >= d_prev

    # Determine bias (overall direction) based on trend and location
    bias = "neutral"
    if up_trend:
        bias = "long"
    if down_trend:
        bias = "short"
    if bias == "long" and near_sr_short:
        bias = "neutral"
    if bias == "short" and near_sr_long:
        bias = "neutral"

    # Determine triggers: long or short triggers independent of bias
    long_trig = ((pin and getattr(pin, "direction", "") == "long") or
                 (eng and getattr(eng, "direction", "") == "long") or
                 mom_long or sweep == "long")
    short_trig = ((pin and getattr(pin, "direction", "") == "short") or
                  (eng and getattr(eng, "direction", "") == "short") or
                  mom_short or sweep == "short")

    direction = "neutral"
    if bias == "long" and long_trig:
        direction = "long"
    elif bias == "short" and short_trig:
        direction = "short"
    else:
        # If only one side triggers, allow that direction even if bias neutral
    ...
    close = closes[-1]
    # Compute stop and targets using ATR risk model
    if direction == "long":
        stop = min(lows[-1], lows[-2])
        stop = min(stop, close - a14[-1] * 0.8)
        tp1 = close + (close - stop) * 1.5
        tp2 = close + (close - stop) * 2.0
    else:
        stop = max(highs[-1], highs[-2])
        stop = max(stop, close + a14[-1] * 0.8)
        tp1 = close - (stop - close) * 1.5
        tp2 = close - (stop - close) * 2.0

    rr = 0.0
    if close != stop:
        rr = round(abs((tp1 - close) / (close - stop)), 2)

    # Confluence summary list
    conf = []
    if up_trend:
        conf.append("Trend up")
    if down_trend:
        conf.append("Trend down")
    if near_sr_long or near_sr_short:
        conf.append("At HTF S/R")
    if sweep:
        conf.append(f"Liquidity sweep ({sweep})")
    if fvg:
        conf.append("FVG context")
    if pin:
        conf.append("Pin bar")
    if eng:
        conf.append("Engulfing")
    if mom_long or mom_short:
        conf.append("Stoch RSI exit from OB/OS")
    # Deduplicate while preserving order
    conf = list(dict.fromkeys(conf))

    # Simple confidence scoring out of 100
    score = 60
    for key in ["Trend up", "Trend down", "At HTF S/R", "Liquidity sweep (long)", "Liquidity sweep (short)", "FVG context", "Pin bar", "Engulfing", "Stoch RSI exit from OB/OS"]:
        if any(key in c for c in conf):
            score += 5
    score = max(0, min(95, score))

    # Timing information: placeholder; you can enhance to detect session/volatility
    timing_text = "Session OK; volatility normal."

    # News filter: check if any high‑impact event on either currency in next 60 minutes
    news_text = None
    base_ccy = pair[:3].upper() if len(pair) >= 6 else ""
    quote_ccy = pair[3:6].upper() if len(pair) >= 6 else ""
    if news_events:
        # Convert last timestamp to datetime; times list is ISO format with 'Z'
        try:
            now = dt.datetime.fromisoformat(times[-1].replace("Z", "+00:00"))
        except Exception:
            now = dt.datetime.utcnow()
        window = dt.timedelta(minutes=60)
        for ev in news_events:
            if ev["impact"].lower() in ("high", "red") and ev["ccy"] in (base_ccy, quote_ccy):
                if abs((ev["time"] - now).total_seconds()) <= window.total_seconds():
                    mins = int((ev["time"] - now).total_seconds() / 60)
                    news_text = f"{ev['ccy']} {ev['title']} in ~{mins}m"
                    break

    # Narrative sections: select from multiple phrase templates to humanize the output.
    # Define a few voices for long and short setups. Each voice provides templates for bias, plan and notes.
    long_voices = [
        {
            'bias': "Uptrend remains intact, with EMAs stacked and rising. Watching for a pullback into support to rejoin the trend.",
            'plan': "Entry {entry:.5f} on a dip; Stop below {stop:.5f}; TP1 {tp1:.5f}, TP2 {tp2:.5f}; R:R about {rr:.1f}.",
            'notes': "I'll trail after the first target or exit if price breaks below support."
        },
        {
            'bias': "Bulls are in control. Higher highs and lows plus a rising 20- and 50-period EMA point to continued strength. After a healthy retrace into a demand zone, I'm favouring longs.",
            'plan': "I'll look to get long near {entry:.5f}, with a protective stop under the last swing at {stop:.5f}. I'll take partial profits near {tp1:.5f} and leave a runner toward {tp2:.5f}, targeting a roughly {rr:.1f}:1 reward-to-risk.",
            'notes': "As price moves in my favor, I'll move the stop to break-even and watch lower timeframes for confirmation. If price closes back below the invalidation zone, I'll stand aside."
        },
        {
            'bias': "Market structure is bullish, evidenced by higher highs and a rising EMA50. Price has pulled back toward a confluence of support around the recent swing lows, hinting at a long setup.",
            'plan': "My strategy is to place a limit order around {entry:.5f}, with a stop tucked below the invalidation point at {stop:.5f}. I'll target the recent high at {tp1:.5f} and the measured extension toward {tp2:.5f}, giving me an attractive {rr:.1f}:1 risk-to-reward.",
            'notes': "I'll manage risk by locking in profits at the first target and trailing the remainder. If there are significant news events or if price fails to hold above the pullback level, I'll reassess."
        }
    ]
    short_voices = [
        {
            'bias': "Downtrend continues with EMAs stacked and falling. Looking to sell rallies into resistance.",
            'plan': "Entry {entry:.5f} on a bounce; Stop above {stop:.5f}; TP1 {tp1:.5f}, TP2 {tp2:.5f}; R:R about {rr:.1f}.",
            'notes': "I'll trail after the first target or exit if price breaks above resistance."
        },
        {
            'bias': "Bears hold control. Lower highs and lows with a downward EMA stack signal continued weakness. After a retrace into a supply zone, I'm favouring shorts.",
            'plan': "I'll look to get short near {entry:.5f}, with a protective stop above the last swing high at {stop:.5f}. I'll take some profits near {tp1:.5f} and aim for {tp2:.5f} beyond that, targeting roughly {rr:.1f}:1 reward-to-risk.",
            'notes': "I'll move the stop to break-even after TP1 hits and monitor momentum; if price closes above my invalidation zone, I'll step aside."
        },
        {
            'bias': "The market structure is bearish, evidenced by lower lows and a falling EMA50. Price has retraced toward a confluence of resistance near the recent highs, suggesting a short opportunity.",
            'plan': "My plan is to sell near {entry:.5f}, with a stop loss just above the invalidation level at {stop:.5f}. I'll target the prior low at {tp1:.5f} first and a measured extension to {tp2:.5f}, offering a {rr:.1f}:1 risk/reward.",
            'notes': "I'll manage the position by locking in profits at the first target and trailing the remainder lower. If there's a bullish reversal or major news, I'll close the trade."
        }
    ]
    if direction == 'long':
        voice = random.choice(long_voices)
        bias_text = voice['bias']
        plan_text = voice['plan'].format(entry=close, stop=stop, tp1=tp1, tp2=tp2, rr=rr)
        notes_text = voice['notes']
    else:
        voice = random.choice(short_voices)
        bias_text = voice['bias']
        plan_text = voice['plan'].format(entry=close, stop=stop, tp1=tp1, tp2=tp2, rr=rr)
        notes_text = voice['notes']

    return AIIdea(
        pair=pair, tf=tf, direction=direction, entry=close, stop=stop,
        targets=[tp1, tp2], rr=rr, confluences=conf, bias_text=bias_text,
        plan_text=plan_text, timing_text=timing_text, news_text=news_text,
        notes_text=notes_text, confidence=score
    )
...
def det_astra1_strategy(pair: str, tf: str, highs, lows, opens, closes, times) -> Optional[Detected]:
    ...
    # Updated proprietary strategy tags: use a single tag for this strategy
    return Detected("AstraFX Strategy #1", direction, entry, stop, score, tags=["Proprietary #1"])

def det_user_confluence(pair: str, tf: str, highs, lows, opens, closes, times, items: List[str]) -> Optional[Detected]:
    """
    Evaluate a user‑defined confluence on the last closed bar.
    Conditions are grouped into two categories:

      • **Candlestick patterns** (Pinbar, Engulfing, Hammer / Shooting star,
        Morning star / Evening star, Hanging man). When multiple candle items
        are selected, only one needs to be present (logical OR).

      • **Context conditions** (all other items). All selected context
        conditions must hold (logical AND).

    Returns a signal when at least one selected candle condition is true
    (if any were selected) and all selected context conditions are true. If
    no candle conditions are selected, only the context conditions are
    evaluated.

    Direction is chosen in order of explicit candle signals, then
    liquidity sweep, then trend slope. When no explicit direction exists,
    defaults to long.

    Score is scaled by the number of selected items (capped at 95) so
    user‑defined confluence cards integrate naturally with the Min Score
    slider.
    """
    ...
    # Trend via EMA50 slope
    slope50 = e50[-1] - e50[-5] if len(e50) >= 5 else 0.0
    trending_dir = "long" if slope50 > 0 else "short" if slope50 < 0 else None
    # Support/resistance proximity
    hh, ll = _swing_levels(highs, lows, 50)
    sr_tol = max(a14[-1] * 0.5, closes[-1] * 0.001)
    at_sr_long = _near(closes[-1], ll, sr_tol)
    at_sr_short = _near(closes[-1], hh, sr_tol)
    # Location helpers
    fib_ok = _fib_overlap(closes[-1], highs, lows, 50, 0.002)
    round_ok = _round_level_proximity(closes[-1], pair)
    # Liquidity sweep and FVG context
    sweep_dir = _liquidity_sweep(highs, lows, closes, 10)
    has_fvg, fvg_dir = _fvg_context(highs, lows)
    # Stochastic RSI cross as a momentum qualifier
    try:
        K, D_vals, _ = _stoch_rsi_kd(closes, 14, 14, 3, 3)
        k1 = K[-1] if K else 50.0
        d1 = D_vals[-1] if D_vals else 50.0
        k0 = K[-2] if len(K) > 1 else k1
        d0 = D_vals[-2] if len(D_vals) > 1 else d1
        stoch_cross = (k0 < 20 and k1 > 20 and k1 > d1) or (k0 > 80 and k1 < 80 and k1 < d1)
    except Exception:
        stoch_cross = False
    # Candlestick detectors
    d_pin = detect_pinbar(highs, lows, opens, closes)
    d_eng = detect_engulfing(highs, lows, opens, closes)
    # Morning/evening star helper
    def _morning_evening_dir() -> Optional[str]:
        if len(closes) < 3:
            return None
        ...
        b1 = abs(c1 - o1)
        b2 = abs(c2 - o2)
        # Morning star: bearish → small inside bar → bullish closing above midpoint
        if (c1 < o1) and (b1 > b2 * 1.5) and (min(o2, c2) >= min(o1, c1) and max(o2, c2) <= max(o1, c1)):
            mid1 = (o1 + c1) * 0.5
            if (c3 > o3) and (c3 > mid1):
                return "long"
        # Evening star: bullish → small inside bar → bearish closing below midpoint
        if (c1 > o1) and (b1 > b2 * 1.5) and (min(o2, c2) >= min(o1, c1) and max(o2, c2) <= max(o1, c1)):
            mid1 = (o1 + c1) * 0.5
            if (c3 < o3) and (c3 < mid1):
                return "short"
        return None
    me_dir = _morning_evening_dir()
    # Hanging man: bearish pin bar within an uptrend
    hanging_ok = False
    if d_pin and trending_dir == "long" and d_pin.direction == "short":
        hanging_ok = True
    # Liquidity & Imbalance: equal highs/lows or FVG presence
    liq_imb = False
    try:
        tol_liq = a14[-1] * 0.2
    except Exception:
    ...
    for i in range(-10, -1):
        ...
        if j < -len(highs):
            break
        if abs(highs[i] - highs[j]) < tol_liq or abs(lows[i] - lows[j]) < tol_liq:
            liq_imb = True
            break
    if has_fvg:
        liq_imb = True
    # Prior day and pivot proximity
    prior_ok = False
    pivot_ok = False
    try:
        ...
        for t in reversed(times):
            if not t:
                continue
            try:
                tt = str(t).replace("T", " ").replace("Z", "")
                dt_last = _datetime.datetime.fromisoformat(tt)
                break
            except Exception:
                continue
        if dt_last is not None:
            prev_date = dt_last.date() - _datetime.timedelta(days=1)
            hi_prev = None
            lo_prev = None
            close_prev = None
            for h, l, o, c, t in zip(highs, lows, opens, closes, times):
                if not t:
                    continue
                try:
                    tt = str(t).replace("T", " ").replace("Z", "")
                    d = _datetime.datetime.fromisoformat(tt).date()
                except Exception:
                    continue
                if d == prev_date:
                    if hi_prev is None or h > hi_prev:
                        hi_prev = h
                    if lo_prev is None or l < lo_prev:
                        lo_prev = l
                    close_prev = c
            if hi_prev is not None and lo_prev is not None and close_prev is not None:
                price = closes[-1]
                tol_pd = max(a14[-1] * 0.5, price * 0.001)
                prior_ok = _near(price, hi_prev, tol_pd) or _near(price, lo_prev, tol_pd)
                P = (hi_prev + lo_prev + close_prev) / 3.0
                R1 = 2 * P - lo_prev
                S1 = 2 * P - hi_prev
                pivot_ok = _near(price, P, tol_pd) or _near(price, R1, tol_pd) or _near(price, S1, tol_pd)
    except Exception:
        prior_ok = False
        pivot_ok = False
    # Consolidate condition booleans
    conds: Dict[str, bool] = {
        "Trend and structure": bool(slope50),
        "D1/H4 bias": (len(e200) > 0 and (e50[-1] != e200[-1])),
        "20/50 EMA slope/stack": ((e20[-1] > e50[-1] and slope50 > 0) or (e20[-1] < e50[-1] and slope50 < 0)),
        "Location": (fib_ok or round_ok or at_sr_long or at_sr_short),
        "HTF S/R": (at_sr_long or at_sr_short),
        "Supply - Demand zones": (at_sr_long or at_sr_short),
        "FIB overlap": fib_ok,
        "Round levels": round_ok,
        "Prior day high or low": prior_ok,
        "Classic pivots": pivot_ok,
        "Liquidity & Imbalance": liq_imb,
        "Liquidity sweep": (sweep_dir is not None),
        "Fair Value Gap": has_fvg,
        "Momentum Confirmation": stoch_cross,
        "Pinbar": (d_pin is not None),
        "Engulfing": (d_eng is not None),
        "Hammer / Shooting star": (d_pin is not None),
        "Morning star / Evening star": (me_dir is not None),
        "Hanging man": hanging_ok,
    }
    # Define which items are candle patterns
    candle_items = {
        "Pinbar", "Engulfing", "Hammer / Shooting star",
        "Morning star / Evening star", "Hanging man"
    }
    selected = set(items)
    selected_candles = selected & candle_items
    selected_context = selected - candle_items
    # Check all selected context conditions (AND)
    for it in selected_context:
        if not conds.get(it, False):
            return None
    # Check candle conditions: require any selected candle to be true
    if selected_candles:
        ok = False
        for it in selected_candles:
            if conds.get(it, False):
                ok = True
                break
        if not ok:
            return None
    # Determine direction: prefer explicit candle or sweep signals
    direction: Optional[str] = None
    # Pinbar direction
    if d_pin and "Pinbar" in selected_candles:
        direction = d_pin.direction
    # Engulfing direction
    if (direction is None) and d_eng and "Engulfing" in selected_candles:
        direction = d_eng.direction
    # Hammer/shooting star reuses pin bar
    if (direction is None) and d_pin and "Hammer / Shooting star" in selected_candles:
        direction = d_pin.direction
    # Morning/evening star
    if (direction is None) and me_dir and "Morning star / Evening star" in selected_candles:
        direction = me_dir
    # Hanging man overrides direction as short
    if (direction is None) and hanging_ok and "Hanging man" in selected_candles:
        direction = "short"
    # Sweep or FVG direction
    if direction is None and sweep_dir:
        direction = sweep_dir
    if direction is None and has_fvg and fvg_dir:
        direction = fvg_dir
    if direction is None:
        direction = trending_dir or "long"
    # Build result
    entry = closes[-1]
    stop = lows[-1] - a14[-1] * 0.25 if direction == "long" else highs[-1] + a14[-1] * 0.25
    score = min(95.0, 78.0 + 2.0 * len(items))
...
def gather_candidates(pair: str, tf: str, series: List[Dict[str, float]], which_pattern: str, which_strategy: str) -> Dict[str, List[Detected]]:
    ...
    strat: List[Detected] = []
    # Only compute built‑in strategy detections when the user hasn't selected "none"
    if ws != "none":
    ...
    pat: List[Detected] = []
    # Only compute pattern detections when the user hasn't selected "none"
    if wp != "none":
...
def api_scan(
    ...
    """
    Scan for signals across one or more currency pairs and return setups. If `ai=1`,
    the API returns only discretionary AI ideas and skips all other detection routines.

    Parameters:
      pairs: Comma‑separated list of currency symbols to scan.
      tf: Time frame (e.g. "M15", "H1").
      min_score: Minimum confidence score required to include a signal (ignored for AI ideas).
      ...
      udc: Comma‑separated list of user‑defined confluence items. Ignored when AI mode is on.
      ai: Set to "1" to enable AI‑Trading Ideas mode. When enabled, only AI ideas are returned.
    ...
    ai_flag = (ai == "1")
    # Load news events once if AI mode enabled
    news_events: List[Dict[str, Any]] = load_local_news_events() if ai_flag else []
    # Normalize proprietary key once for comparisons
    prop_key = (prop or "none").lower()
    # Parse user‑defined confluence selections once
    udc_items: List[str] = []
    if udc and not ai_flag:
        udc_items = [x.strip() for x in udc.split(",") if x.strip()]
    for p in [x.strip() for x in pairs.split(",") if x.strip()]:
        try:
            series = fetch_ohlc_router(p, tf, 220)
            # Extract OHLC arrays once per pair
            opens  = [x["open"] for x in series]
            highs  = [x["high"] for x in series]
            lows   = [x["low"] for x in series]
            closes = [x["close"] for x in series]
            ...
            if ai_flag:
                # AI mode: generate discretionary idea and skip other detections
                idea = det_ai_trading_idea(p, tf, highs, lows, opens, closes, times, news_events)
                if idea:
                    # Convert tf to tradingview interval code
                    tv_tf = {"M5": "5", "M15": "15", "M30": "30", "H1": "60", "H4": "240", "D1": "D"}.get(tf, "60")
                    tv_sym = f"OANDA:{p}"
                    ...
                    results.append({
                        ...
                        "tps": [float(x) for x in idea.targets],
                        "tv": tv_link,
                        "bias_text": idea.bias_text,
                        "plan_text": idea.plan_text,
                        "confluence_list": idea.confluences,
                        "timing_text": idea.timing_text,
                        "news_text": idea.news_text,
                        "notes_text": idea.notes_text
                    })
                # Skip other detection routines entirely when AI is on
                continue
            ...
            if base:
                results.extend(base)
            # Proprietary strategy detection
            if prop_key != "none":
                if prop_key == "astra1":
                    d = det_astra1_strategy(p, tf, highs, lows, opens, closes, times)
                    if d and d.score >= min_score:
                        results.append(d.to_signal(p, tf))
            # User‑defined confluence detection
            if udc_items:
                d_udc = det_user_confluence(p, tf, highs, lows, opens, closes, times, udc_items)
                if d_udc and d_udc.score >= min_score:
                    results.append(d_udc.to_signal(p, tf))
        except Exception as e:
...
def _bars_per_day(tf: str) -> int:
    return {"M5":288, "M15":96, "M30":48, "H1":24, "H4":6, "D1":1}.get(tf.upper(), 24)

def _detect_once(group: str, signal_key: str, pair: str, tf: str, highs, lows, opens, closes, times, udc_items):
    # returns Optional[Detected]
    if group == "strategy":
        if signal_key == "ichimoku":
            return det_ichimoku_strategy(highs, lows, closes)
        if signal_key == "ema_pullback":
            return det_ema_pullback_strategy(highs, lows, closes)
        if signal_key == "bbsqueeze":
            return det_bbsqueeze_strategy(highs, lows, closes)
        if signal_key == "donchian":
            return det_donchian_strategy(highs, lows, closes)
        if signal_key == "orb_london":
            return det_orb_london_strategy(highs, lows, closes)
        if signal_key == "rsi2":
            return det_rsi2_strategy(highs, lows, closes)
    elif group == "pattern":
        if signal_key == "pinbar":
            return detect_pinbar(highs, lows, opens, closes)
        if signal_key == "engulfing":
            return detect_engulfing(highs, lows, opens, closes)
        if signal_key == "insidebar":
            return detect_insidebar(highs, lows, opens, closes)
        if signal_key == "nr7":
            return detect_nr7(highs, lows, closes)
    elif group == "prop":
        if signal_key == "astra1":
            return det_astra1_strategy(pair, tf, highs, lows, opens, closes, times)
    elif group == "udc":
        return det_user_confluence(pair, tf, highs, lows, opens, closes, times, udc_items or [])
    return None

def _target_from_exit(d: 'Detected', exit_rule: str):
    risk = abs(d.entry - d.stop)
    mult = 1 if exit_rule.upper() == "TP1" else 2 if exit_rule.upper() == "TP2" else 3
    if d.direction == "long":
        return d.entry + risk * mult
    else:
        return d.entry - risk * mult

@app.get("/api/tester")
def api_strategy_tester(
    pair: str = Query(..., description="Symbol e.g., EURUSD"),
    tf: str = Query("H1", description="M5,M15,M30,H1,H4,D1"),
    lookback_days: int = Query(30, ge=1, le=90),
    min_score: int = Query(70, ge=0, le=100),
    exit_rule: str = Query("TP1"),
    group: str = Query("strategy", description="strategy|pattern|prop|udc"),
    signal: str = Query("ichimoku", description="key for strategy/pattern/prop"),
    udc: str = Query("", description="comma separated for group=udc")
):
    # Fetch OHLC large enough to cover lookback
    bars_needed = _bars_per_day(tf) * lookback_days + 100
    series = fetch_ohlc_router(pair, tf, bars_needed)
    highs  = [x["high"] for x in series]
    lows   = [x["low"] for x in series]
    opens  = [x["open"] for x in series]
    closes = [x["close"] for x in series]
    ...
    udc_items = [x.strip() for x in udc.split(",") if x.strip()] if group == "udc" else []

    # Iterate through historical bars (ignore very early bars where indicators require warmup)
    start_idx = max(60, len(series) - _bars_per_day(tf)*lookback_days - 1)  # leave last as forming
    wins = 0; losses = 0; trades = []
    for i in range(start_idx, len(series)-1):
        sub_hi, sub_lo = highs[:i+1], lows[:i+1]
        sub_op, sub_cl = opens[:i+1], closes[:i+1]
        sub_tm = times[:i+1]
        d = _detect_once(group, signal, pair, tf, sub_hi, sub_lo, sub_op, sub_cl, sub_tm, udc_items)
        if not d or d.score < min_score:
            continue

        # compute TP from chosen exit rule
        tp = _target_from_exit(d, exit_rule)
        entry = d.entry; stop = d.stop; direction = d.direction
        ...
        for j in range(i+1, len(series)):
            ...
            if hit_stop and hit_tp:
                # tie-breaker: whichever is closer to entry assumed hit first
                if abs(tp - entry) <= abs(entry - stop):
                    hit_stop = False
                else:
                    hit_tp = False
            if hit_tp:
                outcome = "win"; wins += 1; bars_held = j - i; break
            if hit_stop:
                outcome = "loss"; losses += 1; bars_held = j - i; break
        if outcome:
            trades.append({
                "time": times[i],
                "direction": direction,
    ...
    total = wins + losses
    win_pct = round((wins/total*100.0) if total else 0.0, 1)
    result = {
        "summary": {
            "group": "User Defined Confluence" if group == "udc" else ("AstraFX Proprietary" if group == "prop" else group.upper()),
            "signal": ", ".join(udc_items) if group == "udc" else signal,
            "timeframe": tf,
            "exit_rule": exit_rule.upper(),
            "min_score": min_score,
            "lookback_days": lookback_days
        },
        "results": {
            "pair": pair, "trades": total, "wins": wins, "losses": losses, "win_pct": win_pct
        },
        "trades": trades
    }
    return result
...
# === AstraFX: JSON Strategy Loader (auto-discovery) ============================
from typing import List as _List, Dict as _Dict, Any as _Any, Callable as _Callable
from fastapi import Body
import importlib.util as _importlib_util

STRATEGY_DIR_PATH = os.path.join(os.path.dirname(__file__), "strategies")
os.makedirs(STRATEGY_DIR_PATH, exist_ok=True)

def _load_strategy_meta() -> _List[_Dict[str,_Any]]:
    metas = []
    for fn in os.listdir(STRATEGY_DIR_PATH):
        if fn.endswith(".json"):
            try:
                with open(os.path.join(STRATEGY_DIR_PATH, fn), "r") as f:
                    metas.append(json.load(f))
            except Exception as e:
                print("Failed loading strategy JSON:", fn, e)
    return metas

def _load_runner_py(module_name: str):
    py_path = os.path.join(STRATEGY_DIR_PATH, module_name)
    if not os.path.exists(py_path):
        return None
    spec = _importlib_util.spec_from_file_location(module_name.replace(".py",""), py_path)
...
def api_strategies():
    return _load_strategy_meta()

@app.post("/api/tester_json")
def api_tester_json(payload: dict = Body(...)):
    # expected: { "pair": "EURUSD", "timeframe": "M15", "strategy_id": "...", "params": {} }
    sid = payload.get("strategy_id")
    if not sid:
        return {"error": "strategy_id required"}
    metas = {m["id"]: m for m in _load_strategy_meta() if "id" in m}
    if sid not in metas:
        return {"error": f"strategy_id not found: {sid}"}
    meta = metas[sid]
    params = dict(meta.get("indicators", {}))
    params.update(payload.get("params", {}))

    # Locate runner .py matching convention runner_<id>.py OR explicit mapping
    # For this drop, we look for 'runner_divergence_srsi.py' to match divergence_srsi_v1
    ...
    if not mod or not hasattr(mod, "run_backtest"):
        return {"error": f"Runner not found or invalid for {sid}"}
...
def api_scan_json(payload: dict = Body(...)):
    sid = payload.get("strategy_id")
    if not sid:
        return {"error": "strategy_id required"}
    metas = {m["id"]: m for m in _load_strategy_meta() if "id" in m}
